# GenAIFeb26
## Repository Created for sharing Code with TestLeaf Course on GenAI February'26
Quote:
* __Engineering is everything, but it should be learned by doing__
* Discuss your issues until you are in sync with each other or until you understand each other’s positions and can determine what should be done. As someone I worked with once explained, “It’s simple—just don’t filter.” #principleoftheday
[Source](Futureofai.mit.edu) In a bottom up world of chaos what's useful 
  - Intuition is useful 
  - Flexibility is useful 
  - Speed is useful 
* You need to create a basic model of the world which is achieved by just observing it.